...
/**
 * Normalize a state value to the canonical full state name used across the storefront.
 * Supports exact names, case-insensitive names, and 2-letter abbreviations.
 * @param {string} value - The input state value.
 * @returns {string} The canonical state name or empty string if invalid.
 */
export function normalizeStateName(value) {
  ...
  const exactMatch = ALL_STATES.find(
    (stateName) => stateName.toLowerCase() === decoded.toLowerCase()
  );
  if (exactMatch) {
    return exactMatch;
  }

  return ABBREVIATION_TO_STATE[decoded.toLowerCase()] || "";
}
...
function getStateFromResultsPath(urlObj) {
  ...
  if (!decodedSegment || decodedSegment.toLowerCase() === "all") return "";
  if (isValidState(decodedSegment)) return decodedSegment;
...
export function resolveSelectedStateFromLocation({
  ...
  if (isHomePage) return hashState || fallbackState;
