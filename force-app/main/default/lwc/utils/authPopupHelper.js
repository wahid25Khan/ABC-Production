...
export function isPopupLoginErrorUrl(popupUrl, loginActionUrl) {
    ...
    if (popupUrl.pathname.includes('/_nc_external/identity/sso/ui/AuthorizationError')) {
        return true;
    }

    const providerErrorParams = popupUrl.searchParams;
    if (
        providerErrorParams.has('ErrorCode') ||
        providerErrorParams.has('ErrorDescription') ||
        providerErrorParams.has('ProviderId')
    ) {
        return true;
    }
    ...
    return providerErrorParams.has('error')
        || providerErrorParams.has('loginError')
        || providerErrorParams.has('ec');
}
