...
export default class MapPlusTestimonial extends LightningElement {
  ...
  get mapSlideCardClass() {
    return `${this.cardClass} map-card--carousel`;
  }
  ...
  get leadingTestimonials() {
    return this.hasTestimonials ? this.formattedSlides.slice(0, 1) : [];
  }

  get trailingTestimonials() {
    return this.hasTestimonials ? this.formattedSlides.slice(1) : [];
  }
  ...
  handleExternalStateChange(event) {
    ...
    const nextState = this.getValidStateName(value);
