...
const LEGEND_STEP = 5;
const LEGEND_COLOR_SCALE = ['#e2e4e8', '#ece794', '#e8d25f', '#ecac4e', '#d77a3d', '#b34139', '#7a0b2e', '#4f031f'];
...
export default class MapPlusTestimonial extends LightningElement {
    ...
    connectedCallback() {
        ...
        this.fillColorExpression = this.buildFillColorExpression(this.legendEntries);
        this.updateStateFromUrlOrStorage();
        this.applySelectedStateFromContext();
        this.fetchCarouselData();
        this.startHashWatcher();
        this._boundStateEventHandler = (event) => this.handleExternalStateChange(event);
    ...
    disconnectedCallback() {
        ...
        if (this._boundStateEventHandler) {
            window.removeEventListener('abcstatechange', this._boundStateEventHandler);
    ...
    get cardClass() {
        return this.isExpandable ? 'swiper-slide map-card map-card--interactive' : 'swiper-slide map-card';
    }

    get stateAbbreviation() {
        const resolvedFips = this.resolveStateFips(String(this.selectedState || '').toUpperCase());
        return STATE_FIPS_TO_ABBR[resolvedFips] || String(this.selectedState || '').slice(0, 2).toUpperCase();
    }
    ...
    get legendRows() {
        const source = this.legendEntries?.length ? this.legendEntries : this.getLegendEntriesForMax(DEFAULT_LEGEND_MAX);
    ...
    get hasData() {
        return Array.isArray(this.formattedSlides) && this.formattedSlides.length > 0;
    }
    ...
    applySelectedStateFromContext() {
        ...
        const resolved = this.getValidStateName(urlState || storedState || this._selectedState);
        this._selectedState = resolved;
    }

    handleExternalStateChange(event) {
        const value = String(event?.detail?.state || event?.detail?.selectedState || '').trim();
        if (!value) {
            return;
        }
    ...
    hideMapboxBranding(map) {
        ...
        const compactControlGroup = container.querySelector('.mapboxgl-ctrl-bottom-left');
        if (compactControlGroup && !compactControlGroup.querySelector('.mapboxgl-ctrl')) {
            compactControlGroup.style.display = 'none';
        }
    }

    loadMapboxAssets() {
        return Promise.all([this.loadScript(MAPBOX_GL_JS), this.loadStyle(MAPBOX_GL_CSS)]);
    }
    ...
    async refreshRenderedMaps() {
        ...
        try {
            ...
            const featureCollection = this.buildStateFeatureCollection(geoJson, this.selectedState);

            if (!featureCollection.features.length) {
                throw new Error(`No county features found for state: ${this.selectedState}`);
            }

            this.lastFeatureCollection = featureCollection;
            this.legendEntries = this.getLegendEntriesForFeatures(featureCollection.features);
            this.fillColorExpression = this.buildFillColorExpression(this.legendEntries);

            if (this.previewMapLoaded && this.previewMap) {
                this.renderCountyLayers(this.previewMap, featureCollection);
                this.fitToFeatures(this.previewMap, featureCollection.features, ROLE_PREVIEW);
            }

            if (requestId === this.renderRequestId && this.modalMapLoaded && this.modalMap) {
                this.renderCountyLayers(this.modalMap, featureCollection);
                this.fitToFeatures(this.modalMap, featureCollection.features, ROLE_MODAL);
            }
        } catch (error) {
    ...
    hasAnyLoadedMap() {
        return (this.previewMapLoaded && this.previewMap) || (this.modalMapLoaded && this.modalMap);
    }

    async fetchGeoJson() {
        if (!this.geoJsonPromise) {
            const resourceUrl = this.geoJsonPath ? `${US_GEOJSON}/${this.geoJsonPath}` : US_GEOJSON;
            this.geoJsonPromise = fetch(resourceUrl)
                .then((response) => {
                    if (!response.ok) {
                        throw new Error(`Failed to load US_GeoJson static resource. HTTP ${response.status}`);
                    }
    ...
    buildStateFeatureCollection(geoJson, stateName) {
        const targetState = String(stateName || '').trim().toUpperCase();
        const targetStateFips = this.resolveStateFips(targetState);
        const features = (geoJson?.features || [])
            .filter((feature) => this.featureBelongsToState(feature, targetState, targetStateFips))
            .map((feature) => {
    ...
    featureBelongsToState(feature, targetState, targetStateFips) {
        ...
        const stateFips = String(props.STATEFP || props.statefp || props.STATE || props.state || '')
            .trim()
            .padStart(2, '0');
    ...
    getCountyName(feature) {
        const props = feature?.properties || {};
        const rawName = props.NAME || props.name || props.COUNTY || props.county || props.NAMELSAD || 'Unknown';

        return String(rawName).replace(/\s+County$/i, '').trim();
    }

    resolveMetricValue(feature) {
        const props = feature?.properties || {};

        const explicit = Number(props.metricValue || props.METRIC_VALUE || props.value || props.VALUE);
    ...
    renderCountyLayers(map, featureCollection) {
        ...
        map.addLayer({
            ...
            type: 'symbol',
            source: SOURCE_ID,
            maxzoom: map === this.previewMap && this.isCompact ? 24 : this.getCountyLabelMinZoom(map),
            layout: {
                'text-field':
                    map === this.previewMap && this.isCompact
                        ? [
                              'case',
                              ['>', ['coalesce', ['to-number', ['get', 'metricValue']], 0], 10],
                ...
                'symbol-sort-key': ['coalesce', ['to-number', ['get', 'metricValue']], 0]
            },
            paint: {
                'text-color': '#1f2937',
                'text-halo-color': '#fff',
    ...
    buildFillColorExpression(entries) {
        const legendEntries = entries?.length ? entries : this.getLegendEntriesForMax(DEFAULT_LEGEND_MAX);
        const expression = ['step', ['coalesce', ['to-number', ['get', 'metricValue']], 0], legendEntries[0].color];
    ...
    showHoverPopup(map, role, lngLat, feature) {
        ...
        this.hoverPopups[role]
            .setLngLat(lngLat)
            .setHTML(`<strong>${safeCountyName}</strong><br/>District Total: ${safeValue}`)
            .addTo(map);
    }
    ...
    fitToFeatures(map, features, role) {
        ...
        if (coordinates.length) {
            const normalizedCoordinates = this.normalizeCoordinatesForViewport(coordinates);
    ...
    getValidStateName(value) {
        ...
        const exact = Object.keys(STATE_NAME_TO_FIPS).find((stateName) => stateName === normalized);
    ...
    getSelectedStateFromUrl() {
        try {
            ...
            const stateRefinement = parsed.find((item) => item?.nameOrId === STATE_REFINEMENT_KEY);
            if (stateRefinement && Array.isArray(stateRefinement.values) && stateRefinement.values.length) {
