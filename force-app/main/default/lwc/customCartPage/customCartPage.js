...
function isProductCartLine(item) {
  return String(item?.type || "Product").trim().toLowerCase() === "product";
}
...
function mergeCartItems(existingItems, incomingItems) {
  ...
  (incomingItems || []).forEach((incomingItem) => {
    ...
    if (!existingItem) {
      merged.push({ ...incomingItem, cartItemClass: deriveCartItemClass(incomingItem) });
    }
  });
...
export default class CustomCartPage extends LightningElement {
  ...
  wiredCartItems({ data, error }) {
    ...
    this.errorMessage = "";
    const nextItems = extractCartItems(data)
      .filter((item) => isProductCartLine(item))
      .map((item) => normalizeCartItem(item, { storeName: this.storeName }));
  ...
  get isLoading() {
    return this.isLoadingItems || this.isLoadingSummary || this.isLoadingShipping;
  }
  ...
  async commitQuantity(itemId) {
    ...
    if (normalizedQuantity === item.quantity && String(normalizedQuantity) === item.draftQuantity) {
      return;
    }
  ...
  async handleRemoveFromCart(event) {
    ...
    try {
      ...
      await new Promise((resolve) => globalThis.setTimeout(resolve, FADE_OUT_MS));
