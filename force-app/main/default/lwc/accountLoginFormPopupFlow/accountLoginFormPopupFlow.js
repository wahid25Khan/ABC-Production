...
export default class AccountLoginFormPopupFlow extends LightningElement {
    ...
    handleLogin() {
        ...
        try {
            ...
            appendHiddenInput(ownerDocument, form, 'startURL', this.getLoginPostStartUrl());
    ...
    getResolvedStartUrl() {
        ...
        return normalizeInternalUrl(startUrl) || normalizeInternalUrl(this.defaultStartUrl) || '/';
    }
    ...
    doOpenAuthPopup(url, popupName) {
        openAuthPopup(this, url, popupName, {
            ...
            onCompletion: (nextUrl) => {
                const expected = this.getResolvedStartUrl();
                const normalized = normalizeInternalUrl(nextUrl);
                if (normalized === expected || isPopupLoginErrorUrl(new URL(nextUrl, globalThis.location.origin), this.getResolvedLoginActionUrl())) {
