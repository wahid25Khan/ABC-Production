...
async function checkPRStatus(prNumber, repo) {
  ...
  try {
    // Check CI status
    const checks = execSync(`gh pr checks ${prNumber} --repo ${repo}`, {
      encoding: "utf-8",
    ...
    const crReviews = prInfo.reviews.filter(
      (r) => r.author.login === "coderabbitai"
    );
...
async function countOpenCodeRabbitComments(prNumber, repo) {
  ...
  return threads.filter(
    (t) =>
      !t.isResolved && t.comments.nodes[0]?.author?.login === "coderabbitai"
  ).length;
}
  ...
  return pollUntil(
    async () => {
      ...
      return {
        ...
        data: status,
        status:
          `CI: ${status.ciPassed ? "✅" : status.ciPending ? "⏳" : "❌"}, ` +
...
async function autoFixCallback(prNumber, comments) {
  console.log(
    `Attempting to fix ${comments.length} comments for PR #${prNumber}`
  );
  ...
  for (const comment of comments) {
    ...
    if (fix.applied) {
      ...
      await replyToThread(comment.threadId, `${fix.reason}`);
    }
  }
...
onPREvent("pull_request_review.submitted", async (event) => {
  if (
    event.review.user.login === "coderabbitai" &&
...
async function main() {
  ...
  const results = await processStack(prNumbers, repo, {
    ...
    onPRComplete: (pr: number, status: object) =>
      console.log(`✅ PR #${pr} ready!`),
    onPRTimeout: (pr: number, reason: string) =>
      console.log(`⏰ PR #${pr} timed out: ${reason}`),
