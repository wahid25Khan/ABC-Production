...
function detectProjectType() {
  ...
  if (
    fs.existsSync(path.join(ROOT_DIR, "build.gradle")) ||
  ...
  if (
    fs.existsSync(path.join(ROOT_DIR, "requirements.txt")) ||
...
function checkGradle() {
  ...
  const gradleCmd = fs.existsSync(path.join(ROOT_DIR, "gradlew"))
    ? "./gradlew"
    : "gradle";
...
function checkGo() {
  ...
  const fmtResult = runCommand("sh", [
    "-c",
    'find . -name "*.go" -not -path "./vendor/*" | xargs gofmt -l 2>/dev/null'
  ]);
...
function printReport() {
  if (issues.length === 0) {
    if (!QUIET) {
      log(
        `${colors.green}✓${colors.reset} ${colors.dim}${HOOK_TYPE}: All checks passed${colors.reset}`
      );
    }
    return;
  }
  ...
  for (const issue of issues) {
    log(
      `  ${colors.yellow}[!]${colors.reset} ${colors.bold}${issue.category}:${colors.reset} ${issue.message}`
    );
    if (issue.fix) {
  ...
  log(
    `  ${colors.green}These are recommendations. Your commit will proceed.${colors.reset}`
  );
  log(
    `  ${colors.dim}For AI agents: Please address these issues before completing.${colors.reset}`
  );
  log("");
  log("============================================================");
  log("");
