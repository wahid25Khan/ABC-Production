...
export function applyStorefrontRequestParams(
  ...
  nextParams.set("asGuest", asGuest ? "true" : "false");
...
export function buildFreeTrialPath(
  product,
  storeName = DEFAULT_STORE_NAME
) {
  ...
  const params = new URLSearchParams({
    ...
    recordName: recordName || "detail"
  });

  return `/${storeName}/free-trial?${params.toString()}`;
}
...
export async function addProductToCart({
  ...
  try {
    ...
    // addItemToCart can resolve (rather than reject) with an error payload.
    // Detect error shapes: array with errorCode, or object with errorCode/error.
    const errorPayload = Array.isArray(result) ? result[0] : result;
    if (
      errorPayload?.errorCode ||
      errorPayload?.error ||
      (Array.isArray(result) && result.length > 0 && result[0]?.message && !result[0]?.cartItemId)
    ) {
      const msg =
        errorPayload?.message || errorPayload?.error || "Could not add to cart.";
      throw new Error(msg);
    }
...
export function resolveUnitPriceForQuantity(
  variation,
  quantity,
  defaultMinQty = WEBSITE_MINIMUM_QUANTITY
) {
  if (!variation) {
    return null;
  }
  ...
  const qty =
    ...
    enforceWebsiteMinimumQuantity(defaultMinQty);
