...
export function normalizeCartQuantityRule(
  rule,
  fallbackMinimum = WEBSITE_MINIMUM_QUANTITY
) {
  const minimum = enforceWebsiteMinimumQuantity(
...
export function normalizeQuantityToRule(
  value,
  quantityRule,
  fallbackMinimum = WEBSITE_MINIMUM_QUANTITY
) {
...
export function isPhysicalShippableCartItem(item) {
  const explicitDigital = normalizeOptionalBoolean(
    item?.isDigitalProduct ?? item?.isDigital ?? item?.Is_Digital_Product__c
  );

  if (explicitDigital === true) {
    return false;
  }

  if (explicitDigital === false) {
    return true;
  }
  ...
  // Default to shippable when format metadata is absent/ambiguous.
  return true;
}

export function calculateShippableSubtotal(items) {
  return (items || []).reduce((runningTotal, item) => {
    if (!isPhysicalShippableCartItem(item)) {
      return runningTotal;
    }

    const quantity = parsePositiveInteger(item?.quantity) ?? 0;
    const lineTotal =
      toNumber(item?.lineTotal) ?? (toNumber(item?.unitPrice) ?? 0) * quantity;
...
export function buildCartOrderSummaryLines(items) {
  ...
  return [
    summary.digital ? `${summary.digital} Digital Workbooks` : "",
    summary.printed ? `${summary.printed} Printed Workbooks` : "",
...
export function extractCartItems(data) {
  ...
  return rows.map((row) => row?.cartItem || row?.item || row).filter(Boolean);
}
...
export function normalizeCartSummary(data) {
  ...
  const currencyIsoCode = firstString([summary?.currencyIsoCode]) || "USD";
...
export function normalizeCartItem(
  item,
  { storeName = DEFAULT_STORE_NAME, fallbackProductDetails = null } = {}
) {
  const productDetails = item?.productDetails || {};
  const supplementalProductDetails =
    fallbackProductDetails && typeof fallbackProductDetails === "object"
      ? fallbackProductDetails
      : {};
  const productId = String(
    item?.productId ||
      productDetails?.productId ||
      supplementalProductDetails?.productId ||
      supplementalProductDetails?.id ||
      ""
  ).trim();
  ...
  const currencyIsoCode =
    firstString([
      ...
      productDetails?.currencyIsoCode,
      supplementalProductDetails?.currencyIsoCode
    ]) || "USD";
  const quantityRule = normalizeCartQuantityRule(
    productDetails?.purchaseQuantityRule ||
      item?.purchaseQuantityRule ||
      supplementalProductDetails?.purchaseQuantityRule
  );
  const quantity = normalizeQuantityToRule(item?.quantity, quantityRule);
  const isDigitalProduct = normalizeOptionalBoolean(
    item?.isDigitalProduct ??
      item?.isDigital ??
      item?.Is_Digital_Product__c ??
      productDetails?.isDigitalProduct ??
      productDetails?.isDigital ??
      productDetails?.Is_Digital_Product__c ??
      productDetails?.fields?.Is_Digital_Product__c?.value ??
      productDetails?.fields?.Is_Digital_Product__c ??
      supplementalProductDetails?.isDigitalProduct ??
      supplementalProductDetails?.isDigital ??
      supplementalProductDetails?.Is_Digital_Product__c ??
      supplementalProductDetails?.fields?.Is_Digital_Product__c?.value ??
      supplementalProductDetails?.fields?.Is_Digital_Product__c
  );
  const { baseName, formatLabel } = splitCartItemName(
    item?.name || productDetails?.name || supplementalProductDetails?.name
  );
  const lineTotalRaw =
    toNumber(item?.totalPrice) ?? toNumber(item?.totalAmount);
  const derivedUnitPrice =
    lineTotalRaw !== null && quantity > 0 ? lineTotalRaw / quantity : null;
  ...
  const imageUrl = normalizeImageUrl(
    firstString([
      supplementalProductDetails?.thumbnailImage?.url,
      supplementalProductDetails?.thumbnailImage?.thumbnailUrl,
      productDetails?.thumbnailImage?.url,
      productDetails?.thumbnailImage?.thumbnailUrl,
      resolveProductImageUrl(supplementalProductDetails),
  ...
  return {
    ...
    productName:
      baseName ||
      productDetails?.name ||
      supplementalProductDetails?.name ||
      "Product",
    formatLabel,
    isDigitalProduct,
    quantity,
      ...
      ? buildProductDetailPath(
          {
            ...
            name:
              ...
              "detail",
            urlName:
              productDetails?.productUrlName ||
              supplementalProductDetails?.productUrlName ||
              supplementalProductDetails?.urlName
          },
          storeName
        )
      : "",
    imageUrl,
    sku:
      resolveStockKeepingUnit(productDetails) ||
      resolveStockKeepingUnit(supplementalProductDetails),
    unitPrice,
    unitPriceLabel: formatCurrency(unitPrice, currencyIsoCode),
    lineTotal: lineTotalRaw ?? unitPrice * quantity,
    lineTotalLabel: formatCurrency(
      lineTotalRaw ?? unitPrice * quantity,
      currencyIsoCode
    ),
    currencyIsoCode,
    ...
    bulkPricingLabel:
      quantity >= quantityRule.minimum
        ...
        : `+${quantityRule.minimum - quantity} copies for volume discounts`
  };
}
...
async function fetchProductBatch({
  ...
  return batchResults.find((productMap) => productMap.size) || new Map();
}
