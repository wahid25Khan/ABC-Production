...
export default class ProductDetailComponent extends LightningElement {
  ...
  @api minimumQuantity = WEBSITE_MINIMUM_QUANTITY;
  @api trialUrl = "";
  @api showTrialButton;
  ...
  quantity = WEBSITE_MINIMUM_QUANTITY;
  ...
  get displayTiers() {
    const tiers = filterWebsitePricingTiers(this.selectedVariation?.tiers);
  ...
  get minQty() {
    ...
    if (variationMin !== null) return enforceWebsiteMinimumQuantity(variationMin);
    ...
    if (Number.isFinite(ruleMin) && ruleMin > 0) {
      return enforceWebsiteMinimumQuantity(ruleMin);
    }

    return enforceWebsiteMinimumQuantity(this.minimumQuantity);
  }
